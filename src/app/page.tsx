...
type HeroSegment = {type: "text"; text: string} | {type: "tag"; label: string; filter: string};
...
export default function Home() {
  ...
  const filteredCount = useMemo(() => {
    ...
    return ProjectsList.filter((project) => project.tags.some((tag: string) => tag.toLowerCase() === activeFilter.toLowerCase())).length;
  }, [activeFilter]);
          ...
          <h1 className={styles["hero__title"]}>
            {heroSegments.map((segment, index) => {
              ...
              return segment.type === "text" ? (
                <span key={segmentKey} className={styles.hero__word}>
                  {segment.text}
                </span>
              ) : (
                <button key={segmentKey} type="button" onClick={() => toggleFilter(segment.filter)} className={`${styles["hero__tag"]} ${activeFilter === segment.filter ? styles.active : ""}`}>
                  {segment.label}
                </button>
              );
            })}
