    ...
    <div className={styles.project__container}>
      {project.imageUrl && (
        <div className={styles.project__hero}>
          <Image src={project.imageUrl} alt={`Main image for ${project.title}`} width={2560} height={900} priority sizes="100vw" />
        </div>
      )}
          ...
          {project.galleryImages.map((img, index) => (
            <React.Fragment key={img.src}>
              <motion.figure className={styles.project__image} initial="hidden" whileInView="visible" viewport={{once: true, margin: "-100px"}} variants={fadeInUp}>
                <Image src={img.src} alt={img.alt} width={2560} height={900} className={img.className ? styles[img.className] : undefined} sizes="(max-width: 768px) 100vw, (max-width: 1200px) 90vw, 100vw" />
